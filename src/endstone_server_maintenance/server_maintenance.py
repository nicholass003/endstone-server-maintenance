...
class ServerMaintenance(Plugin):
    ...
    def on_load(self) -> None:
        ...
        self._maintenance_players = {"players": []}
    ...
    def on_enable(self) -> None:
        ...
        self.server.scheduler.run_task_timer(
            self, self.maintenance_task, delay=0, period=10
        )
    ...
    def on_command(
        ...
        if not sender.has_permission("server_maintenance.command"):
            sender.send_error_message(
                f"{ColorFormat.RED}You don't have permission to use this command."
            )
            return True
            ...
            case "maintenance":
                if len(args) > 0:
                    ...
                    match subcommand:
                        case "add":
                            if len(args) > 1:
                                if self.add_maintenance_player(args[1].lower()) == True:
                                    sender.send_message(
                                        f"{ColorFormat.GREEN}Successfully added {args[1]} to maintenance players"
                                    )
                                else:
                                    sender.send_error_message(
                                        f"{ColorFormat.RED}Player with gamertag {args[1]} is exists!"
                                    )
                            else:
                                sender.send_error_message(
                                    f"{ColorFormat.RED}Please input player name."
                                )
                        case "on":
                            if (
                                self.handle_activation(
                                    sender=sender, action=self.ACTION_ENABLE, value=True
                                )
                                == True
                            ):
                                ...
                                if len(args) > 1:
                                    ...
                                    if end_value == None:
                                        sender.send_error_message(
                                            f"{ColorFormat.RED}Please input numeric only."
                                        )
                                        return True
                                    self._maintenance["end_date_time"] = (
                                        current_time
                                        + int(
                                            timedelta(hours=end_value).total_seconds()
                                        )
                                    )
                                    sender.send_message(
                                        f"{ColorFormat.RED}Setting Maintenance for {end_value} hour ahead."
                                    )
                                else:
                                    self._maintenance["end_date_time"] = (
                                        current_time
                                    ...
                                    sender.send_message(
                                        f"{ColorFormat.RED}Setting default Maintenance for 1 hour ahead."
                                    )

                                self._maintenance["start_date_time"] = current_time
                        case "off":
                            self.handle_activation(
                                sender=sender, action=self.ACTION_DISABLE, value=False
                            )
                        case "remove":
                            if len(args) > 1:
                                if (
                                    self.remove_maintenance_player(args[1].lower())
                                    == True
                                ):
                                    sender.send_message(
                                        f"{ColorFormat.GREEN}Successfully removed {args[1]} to maintenance players"
                                    )
                                else:
                                    sender.send_error_message(
                                        f"{ColorFormat.RED}Player with gamertag {args[1]} is not exists!"
                                    )
                            else:
                                sender.send_error_message(
                                    f"{ColorFormat.RED}Please input player name."
                        ...
                        case _:
                            sender.send_error_message(
                                f"{ColorFormat.RED}There are no maintenance command with name {args[0]}."
                            )
                else:
                    sender.send_error_message(
                        f"{ColorFormat.RED}Usage: /maintenance [args]."
                    )
                    return True
        return True

    def handle_activation(
        self, sender: CommandSender, action: str, value: bool
    ) -> bool:
        current_state = self.is_maintenance

        if current_state == value:
            sender.send_error_message(
                f"{ColorFormat.RED}Server Maintenance was previously {action}"
            )
            return False
        else:
            ...
            sender.send_message(
                f"{ColorFormat.GREEN}Server Maintenance has been set to {action}."
            )
        return True
